...
class TrainingDataGeneratorANN:
    ...
    def generate_data_image(self) -> Union[list, None]:
        final_values_vector = []
        with self.__mp_pose.Pose(
            min_detection_confidence=self.__detection_confidence,
            min_tracking_confidence=self.__tracking_confidence,
            ...
            self.__vid = cv2.imread(self.__vid)

            image = cv2.resize(self.__vid,(0,0),fx = 0.5, fy = 0.5)
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            image_height, image_width, _ = image.shape
        ...
        return final_values_vector if len(final_values_vector) > 0 else None
